"""result.png: 決算反応の分位別ドリフト(PEAD/モメンタム)とL/S Sharpe。"""
...
# 左: 分位別20日ドリフト(PEAD/継続)
colors=["#c0392b" if v<0 else "#27ae60" for v in a["d20_bps"]]
...
ax1.set_ylabel("決算翌々日から20営業日のTOPIX超過(bps)",fontsize=10)
ax1.set_title("決算で買われた株は続伸・売られた株は続落\n=PEAD(モメンタム継続)、概ね単調",fontsize=10.5,pad=8)
ax1.grid(axis="y",alpha=0.3)
# 右: PEAD L/S Sharpe (q9-q0, 保有別, IS/OOS)
...
ax2.set_ylabel("年率Sharpe(PEAD L/S q9-q0, コスト20bps後)",fontsize=10)
ax2.legend(fontsize=9,title="期間"); ax2.grid(axis="y",alpha=0.3)
ax2.set_title("勝者Long/敗者Shortで20日Sharpe~0.4\nIS/OOS一貫だが昇格基準2.0には遠い",fontsize=10.5,pad=8)
fig.suptitle("決算PEAD(価格反応モメンタム) — 方向は存在するがエッジは弱い・全上場ベータ中立L/S・5年",fontsize=12.5,fontweight="bold",y=0.98)
fig.text(0.99,0.01,"データ: 2021-2026 決算10万件(JQuants fin_summary) / 反応翌々日エントリー / 流動性≥10億円 / TOPIX超過 / 1日あたり中央値3銘柄と薄い",
