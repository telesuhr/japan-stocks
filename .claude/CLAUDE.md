...
### 分析完了 = README + commit + push まで1セット（やりっぱなし禁止）

**1つの分析フォルダを作ったら、必ず以下を完了させてからセッションを終えること。**
README.md未作成・commit未実行・push未実行のままにしない。

```
分析スクリプト実行
    ↓
README.md 作成（結論・数値・解釈を記載）
    ↓
analyses/README.md 目次に追記
    ↓
git add → git commit → git push  ← ここまでが「分析完了」
```
...
### Push タイミング
- コミット後は**即座に Push** する
- セッション中も定期的に Push する（最低1時間に1回）。作業量が多い場合は分析単位でコミット＋Pushしてよい

### セッション末尾の必須作業 (コミット前に必ず実施)
1. **各分析フォルダに `README.md` があることを確認する**（未作成のものは作成してからコミット）
2. **`analyses/README.md` の目次を更新する**
   - 新規分析フォルダを大分類の適切な位置に追記
   - テーマ・主要発見・Sharpe (あれば) を記載
   - 「最新分析」セクションを当日分に差し替える
3. **`analyses/SUMMARY.md` を更新する**（採用/候補/却下に変動があった場合のみ）
4. 上記を含めてまとめてコミット → 即Push
